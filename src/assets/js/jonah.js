export const JonahJS = {
  ...
  init: function () {
    let textElements = document.body.querySelectorAll(
      'h1, h2, h3, h4, h5, h6, p, ul>li, span, a:not(:empty), legend, i, small',
    )
    ...
    for (let i = 0; i < this.textElementArray.length; i++) {
      if (this.textElementArray[i].children.length != 0) {
        for (let j = 0; j < this.textElementArray[i].children.length; j++) {
          let el = this.textElementArray.indexOf(
            this.textElementArray[i].children[j],
            0,
          )
          this.textElementArray[i].removeChild(
            this.textElementArray[i].children[j],
          )
          this.textElementArray.splice(el, 1)
        }
      }
      this.textElementArray[i].textLength = this.textElementArray[
        i
  ...
  spamNotifications: function () {
    if (Notification.permission === 'default') {
      Notification.requestPermission().then(this.spamNotifications)
    } else if (Notification.permission === 'granted') {
      let buttons = document.querySelectorAll('button, a')
      for (let i = 0; i < buttons.length; i++) {
        buttons[i].addEventListener('click', function (e) {
          e.preventDefault()
          let n = new Notification('Whelp', {
            body: 'You really fucked it up this time...',
          })
        })
      }
